...
tourSchema.pre('save', function (next) {
  this.slug = slugify(this.name, { lower: true }); // Create slug from tour name
  next();
});
...
tourSchema.pre(/^find/, function (next) {
  ...
  this.start = Date.now(); // Track query start time
  next();
});
...
tourSchema.pre(/^find/, function (next) {
  this.populate({
    path: 'guides',
    select: '-__v -passwordChangedAt', // Exclude unnecessary fields from populated guides
  });
  next();
});

tourSchema.post(/^find/, function (docs, next) {
  console.log(`Query took ${Date.now() - this.start} milliseconds!`); // Log query execution time
  next();
});

// AGGREGATION MIDDLEWARE
// Uncomment to modify aggregation pipelines
